...
public class Check_API_OpenDay {
    public static void checkResponse() {

        StringBuilder response = sendToAmo("{\"name\":\"Supertester_OpenDay_MZPO\"}");
        ...
        if (status == 0) {
            throw new AssertionError("Тест Supertester_OpenDay_MZPO упал: статус " + status); // Если статус 0, выбрасываем AssertionError
        } else if (status == 1 || status == 2) {
            // Тест считается пройденным при статусах 1 и 2
            System.out.println("Тест Supertester_OpenDay_MZPO прошел успешно: статус " + status);
